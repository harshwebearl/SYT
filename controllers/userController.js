...
module.exports = class usercontroller extends BaseController {
  ...
  async loginAll(req, res) {
    ...
    try {
      ...
      debug(`Debug: Querying userschema for phone: ${phone}, role: ${role}`);
      let userData = await userschema.findOne({ phone, role });
      debug(`Debug: User data retrieved: ${JSON.stringify(userData)}`);

      // Check if the user is found
      if (!userData) {
        debug('Debug: User not found with given phone and role');
